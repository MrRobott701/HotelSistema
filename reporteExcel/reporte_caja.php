...
foreach ($cajas as $caja) {
  ...
  if (count($tipos) > 0) {
    foreach ($tipos as $tipo) :
      ...
      if ($tipo->id == '1') {
        $ingreso_efectivo = ($total_proceso) + $ingreso_efectivo;
      }
      if ($tipo->id == '1') {
        $ingreso_efectivo_prod = ($total_venta) + $ingreso_efectivo_prod;
      }
    endforeach;
  } else {
    $ingreso = 0;
  ...
  if (count($reporproducts_es) > 0) {
    foreach ($reporproducts_es as $reporproduct_e) :
      $subtotal1 = 0 * $reporproduct_e->precio;
