...
// partiendo de que ya tenemos creado un objecto UserData previamente utilizamos el contexto
	...
	public static function getAllPago(){
		$sql = "select * from ".self::$tablename." where pago=0 and is_active=1 and estado!=0";
		$query = Executor::doit($sql);
		return Model::many($query[0],new UserData());
	}

	public static function getLimpieza(){
		$sql = "select * from ".self::$tablename." where is_admin=3 and is_active=1 and estado!=0";
		$query = Executor::doit($sql);
		return Model::many($query[0],new UserData());
	}
	...
	public static function getLike($q){
		$sql = "select * from ".self::$tablename." where name like '%$q%' and estado!=0";
		$query = Executor::doit($sql);
		return Model::many($query[0],new UserData());
